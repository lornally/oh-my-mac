...
> intel芯片的电脑死在了macos12, 如果上os13, 会各种卡顿


### 特别重要的提示: 不要安装在有数据的卷上, 数据会被清空
* 因此, 应该新建一个卷, 然后安装在上面.
* 特别神奇, 文件竟然没丢, 
  * 位置: Previous Content
  * 但是, 在刚刚安装好的时候, 这个文件是看不到的.
  * 问题定位了:  finder直接/System/Volumes/we/就能看到哦, 但是we磁盘的根目录确没有.
  * kimi答复, finder的sidebar的磁盘不是真正的磁盘, 是过滤过的快捷方式.

### 安装之前

* 最好使用老版本的monterey, 2023年的最好, 后面升级的版本效率还是有问题.
* 记得下载clashx, 并且保留好配置链接, 这已经是一个技术活了, 因为clashx的开发者的链接已经失效了, 只能找别人的链接了.
* 搜狗拼音
* chrome浏览器
* karabiner-element 14, 最新的15不能在macos12上面运行
...
1. 密码可以手动导出为csv
2. 手动导出bookmark(书签管理三点)和密码(密码管理的设置中) 
3. 安装的2个插件: zeroomega, immersive translate
1. 设置:
   1. 外观->⌘ Q退出警告取消
   2. 起始页面 ->继续上次的浏览
   3. 隐私与安全->广告隐私设置, 关闭所有
...
brew install   omnidisksweeper  textmate alt-tab sublime-text  anaconda   vlc notion  zed visual-studio-code the-unarchiver freeplane
...
```sh
#### github秘钥设计
ssh-keygen -t ed25519 -C "2025.1121Monterey@mbp2017"
...
```sh
# 这个安装需要之前的gitconfig, 如果没搞gitconfig那么这个无法成功. 太牛逼啦
brew install git-lfs # 为了顺畅处理大文件, 需要安装这个.
git lfs install  # 这个update global git config
# 如果实在不行, 就去https://git-lfs.com/下载并解包之后执行install.sh
```
...
# 因为3.13依旧有intel上pyside6-deploy的兼容问题, 所以采用3.12
...
pip install pyside6  ipykernel
# 此时不安装Nuitka, 等使用pyside6-deploy来自动安装合适的Nuitka
...
# 编译代码此时会自动下载nuitka
pyside6-deploy s0keydog_widget.py

# 可能ssl会报错, 此时安装并不解决问题
brew install ca-certificates
pip install --upgrade certifi
# 导出到zshrc, 解决问题
echo 'export SSL_CERT_FILE=$(python -c "import certifi; print(certifi.where())" 2>/dev/null || echo "")' >> ~/.zshrc && echo 'export REQUESTS_CA_BUNDLE=$SSL_CERT_FILE' >> ~/.zshrc && echo 'export CURL_CA_BUNDLE=$SSL_CERT_FILE' >> ~/.zshrc
# 更本质的方法会失败
cd /usr/local/Cellar/python@3.12/3.12.12/Frameworks/Python.framework/Versions/3.12/ && sudo ./Install\ Certificates.command
